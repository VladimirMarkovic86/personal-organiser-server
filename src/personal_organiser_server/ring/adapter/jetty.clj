(ns personal-organiser-server.ring.adapter.jetty
...
(defn- ^SslContextFactory ssl-context-factory [options]
  (let [context (SslContextFactory.)]
    ...
    (if-let [include-protocols (options :include-protocols)]
      (.setIncludeProtocols context (into-array String include-protocols)))
    ;(println (org.apache.commons.lang3.builder.ToStringBuilder/reflectionToString context))
    context))
...
(defn ^Server run-jetty
  ...
  :include-protocols    - When :ssl? is true, include these protocols
